...
from sqlalchemy import event, exc, desc
        ...
        if r:
            ...
            if twitter_scraper.get_user_profile(username, u):
                ...
                if not user:
                    ...
                    user.location = u.user_location[:255] if u.user_location else ''
                    user.date_joined=dateparser.parse(u.user_created) if u.user_created else None
                    user.timezone=timezone[:10] if timezone else ''
                    user.website=u.website[:255] if u.website else ''
                    user.user_intro=u.user_description[:255] if u.user_description else ''
...
def check_user(user_queue, pg_dsn, proxy_list):
    ...
    username_list = []
    count = 0
    for idx, tweet in enumerate(session.query(Tweet.user_id).filter(Tweet.user_id > 809980000000000000).group_by(Tweet.user_id).order_by(asc(Tweet.user_id)).yield_per(200)):
